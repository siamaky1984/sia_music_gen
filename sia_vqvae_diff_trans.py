...
def create_dataloaders(midi_folder, batch_size=32, bars_per_sequence=8):
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        flat_inputs = inputs.reshape(-1, embedding_dim)
...
class HierarchicalEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, embedding_dim, num_heads=8):
        super().__init__()
        ...
        # Reduce sequence length through strided convolutions
        ...
        # self.attention = AttentionBlock(latent_dim, num_heads)
    ...
    def forward(self, x):
        ...
        x = self.downsampling(x)  # Reduce sequence length and dimension
        x = x.transpose(1, 2)  # [batch_size, reduced_seq_len, latent_dim]
...
class HierarchicalVQVAE(nn.Module):
    def __init__(
        ...
        input_dim, 
        hidden_dim=512, 
        embedding_dim=8,# Significantly reduced latent dimension
        num_embeddings=512, 
        ...
        self.encoder = HierarchicalEncoder(input_dim, hidden_dim, embedding_dim, num_heads)
...
class BarVQVAE(nn.Module):
    def __init__(self, input_dim, hidden_dim=512, latent_dim=256, num_embeddings=512):
        super().__init__()
        self.input_dim = input_dim
        self.latent_dim = latent_dim
        self.hidden_dim = hidden_dim
...
class LatentDiffusionTransformer(nn.Module):
    def __init__(self, latent_dim=64, d_model=512, nhead=8, num_layers=6, max_seq_len=32):
        ...
        print(f"Latent projection input dim: {latent_dim}, output dim: {d_model}")
    ...
    def forward(self, x, t):
        ...
        # print('shape of batched input', x.shape)
        ...
        t_emb = t_emb.expand(-1, num_bars, -1) ## [batch_size, num_bars, d_model]
...
def train_latent_diffusion(transformer, vae, dataloader, num_epochs=100, device="cuda", save_path=None):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in dataloader:
            ...
            # batch shape: [batch_size, bars_per_sequence, bar_length, 2]
             # batch shape: [batch_size, bars_per_sequence, bar_length, 2]
            batch_size, num_bars, bar_length, features = batch.shape
            ...
            # with torch.no_grad():
            #     # B, S, L, F = batch.shape
            #     # print('B, S, L, F', B, S, L, F)
            #     # batch_size, num_bars = batch.size(0), batch.size(1)
            #     # bars = batch.reshape(-1, batch.size(2), batch.size(3))
            #     # print('bars shape', bars.shape)
            #     # latent_bars = vae.encode(bars)
            #     # print(f"Encoded shape: {latent_bars.shape}")

            #     # # Reshape back to sequence form
            #     # latent_sequences = latent_bars.reshape(batch_size, num_bars, -1)
                                
            #     # Get latent representations from VQVAE
                
            #     batch_flat = batch.reshape(-1, batch.size(-2), batch.size(-1))

            #     print('shape of batch flat', batch_flat.shape)
                
            #     latent_sequences = vae.encode(batch_flat)
            #     # latent_batch shape should now be [batch_size, reduced_seq_len, latent_dim]

            #     latent_batch = latent_sequences.reshape(batch.size(0), -1, vae.latent_dim)
            ...
            with torch.no_grad():
                ...
                # ### process each bar one by one using vqvae
                # for i in range(batch.size(1)):
                #     bar = batch[:,i]
                #     latent = vae.encode(bar)
                #     print(latent.shape)
                #     latent_bars.append(latent)



                # Process all bars in the batch simultaneously
                bars_reshaped = batch.view(batch_size * num_bars, bar_length, features)
                latent_bars = vae.encode(bars_reshaped)

                # print(latent_bars.shape)

                ### stack bars
                # latent_sequence = torch.stach(latent_bars, dim = 1)
                latent_sequence = latent_bars.view(batch_size, num_bars, -1)
                
                # latent_sequences.append(latent_sequence)

            # latent_sequences = torch.cat(latent_sequences, dim=0)

            latent_sequences = latent_sequence 
            # print('shape of latent seq', latent_sequences.shape)



            # Add noise and predict
            t = torch.randint(0, 1000, (batch.size(0),), device=device)
            noisy_latents, noise = diffusion.add_noise(latent_sequences, t)
            # predicted_noise = diffusion(noisy_latents, t)
            predicted_noise = transformer(noisy_latents, t)

            # Clear unused tensors
            # del latent_sequences
            # torch.cuda.empty_cache()
...
if __name__ == "__main__":
    ...
    # Usage example
    midi_folder = '../midi_dataset/piano_maestro/piano_maestro-v2.0.0/2004/'
    sequence_len = 32
    feature_dim = 2
    batch_size = 8
    ...
    mode = 'train_diffusion_trans' # 'train_vqvae' # 'train_diffusion_trans' #'train_vqvae' # 'train_diffusion_trans' #  # # 'train_vqvae' # 'train_diffusion_trans' # 'train_vqvae' # 'train_vae'
    ### 'train_diffusion_trans', 'inference'

    vqvae_model_path = './models/vqvae_model.pth'
    latent_diffusion_model_path='./models/latent_diffusion_model.path'
